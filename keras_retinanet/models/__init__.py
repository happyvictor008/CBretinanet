...
class Backbone(object):
    ...
    def __init__(self, backbone):
        ...
        print('________________________________________________________init__________________________________________')
        self.custom_objects = {
...
def load_model(filepath, backbone_name='resnet50', instancesnumber=None):
    ...
    import keras.models
    from .. import losses
    #backbone(backbone_name).custom_objects.update({'_focal': losses.focal(ny=instancesnumber)})
    #backbone(backbone_name).custom_objects['_focal'] = losses.focal(ny=instancesnumber)
    a=backbone(backbone_name).custom_objects
    a.update({'_focal': losses.focal(ny=instancesnumber)})
    print('12121212121211212121121')
    print(instancesnumber)
    print('12121212121211212121121')
    #model=keras.models.load_model(filepath, custom_objects=a)
    print('return model')
    #print(backbone(backbone_name).custom_objects['_focal'])

    #return model

    #return keras.models.load_model(filepath, custom_objects=backbone(backbone_name).custom_objects)
    return keras.models.load_model(filepath, custom_objects=a)
