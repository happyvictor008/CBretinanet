...
class CocoGenerator(Generator):
    ...
    def cal_instances(self):
        #dataDir='C:/Users/14590/OneDrive/桌面/Victor/dataset/coco'
        #dataType='train2017'
        ...
        for cat_name in cat_nms:
            catId = self.coco.getCatIds(catNms=cat_name)
            imgId = self.coco.getImgIds(catIds=catId)
            annId = self.coco.getAnnIds(imgIds=imgId, catIds=catId, iscrowd=None)
            inst.append(len(annId))
        for j in range(len(cat_nms)):
            for i in range(len(inst)):
                if i==j:
                    print(cat_nms[j],inst[i])
        return inst
