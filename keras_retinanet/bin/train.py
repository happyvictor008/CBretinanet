...
##########################################################################################此处需要传入参数 instances num
def create_models(backbone_retinanet, num_classes, weights, multi_gpu=0,
                  freeze_backbone=False, lr=1e-5, config=None, instancesnumber=None):
   ...
   #instancesnumber = None ########################################################################
    ...
    training_model.compile(
        loss={
            ...
            'classification': losses.focal(ny=instancesnumber)###########################################加入参数 各个instances
        },
...
def create_generators(args, preprocess_image):
    ...
    if args.dataset_type == 'coco':
        ...
        ny=train_generator.cal_instances()###############################################new
        ...
        train_generator = visdatasetGenerator(
    ...
    return train_generator, validation_generator , ny
...
def main(args=None):
    ...
    # create the generators##############################################################################需要在这里加入统计函数
    train_generator, validation_generator, ny = create_generators(args, backbone.preprocess_image)
