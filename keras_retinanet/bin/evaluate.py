...
def create_generator(args, preprocess_image):
    ...
    return validation_generator, ny
...
def main(args=None):
    ...
    generator, ny = create_generator(args, backbone.preprocess_image)
    #print(ny)
    ...
    model = models.load_model(args.model, backbone_name=args.backbone, instancesnumber=ny)
    print('next')
    ...
    if args.dataset_type == 'coco':
        ...
        #for labels, (average_precisions_coco, num_annotations) in average_precisions_coco.items():
            #print('{:.0f} instances of class'.format(num_annotations),
                  #generator.label_to_name(labels), 'with average precision: {:.4f}'.format(average_precisions_coco))
            #total_instances_coco.append(num_annotations)
            #precisions_coco.append(average_precisions_coco)
        ...
        ################
        #print('mAP using COCO way : {:.4f}'.format(sum(precisions_coco) / len(precisions_coco)))
